...
class Bill extends CI_Model{
	...
	public function get_all_bill($userId){
		$this->db->select('instituitions.short_title as iTitle,courses.title as cTitle,bill_heads.title as bTitle,departments.title as dTitle,semesters.title as sTitle, academic_sessions.title as seTitle,bills.date,bills.status,bills.id');
		$this->db->from('bills');
